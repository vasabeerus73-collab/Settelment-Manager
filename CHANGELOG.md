...
## 1.2.0 — Первый стабильный релиз

- Убрана дублирующая шапка интерфейса; исправлены индикаторы населения, морали и строительства.
- В списке построек отображается нехватка ресурсов вместо ошибочного статуса «Доступно».
- Игроки могут принимать просьбы жителей и брать доступные проекты через синхронизацию с активным ГМом.
- Просьбы получили автоматическое изменение морали при выполнении и провале.
- Завершение проектов повышает уровень поселения; провал проекта уровень не уменьшает.
- Добавлены проверки переходов статусов, защита от повторного начисления эффектов и записи в летопись.
- Добавлены стабильный и dev-манифесты, автоматическая сборка ZIP и публикация GitHub Releases.
